 ...
 * starts one `node --test-name-pattern=… test/physics.test.js` per bucket. Same tests, same file, same reporter —
 ...
 *
 * TEST_TIMEOUT is enforced by a parent-process watchdog, independent of Node's
 * test timeout semantics. Execute the test file directly (node:test runs it on
 * import), so the watchdog kills the same process doing the synchronous work.
 */
'use strict';
...
function runProcess(args, timeout = TIMEOUT) {
  return new Promise((resolve) => {
    ...
    child.on('close', code => {
      clearTimeout(timer);
      resolve({ code, out, timedOut, seconds: (Date.now() - t0) / 1000 });
    });
  });
}
async function runWorker(names, i) {
  const pattern = `^(?:${names.map(escape).join('|')})$`;
  const r = await runProcess(['--test-reporter=spec', `--test-name-pattern=${pattern}`, FILE]);
  const num = k => { const m = r.out.match(new RegExp(`^\\s*[ℹi]?\\s*${k} (\\d+)$`, 'm')); return m ? Number(m[1]) : 0; };
  const done = r.out.match(/^[✔✖][^\n]*?(?= \(\d)/gm);
  return { ...r, i, lastName: done && done.length ? done[done.length - 1].slice(2) : null,
    counts: { tests: num('tests'), pass: num('pass'), fail: num('fail'), cancelled: num('cancelled'), skipped: num('skipped') } };
}
...
async function main() {
  ...
  for (const r of results) {
    if (!r.timedOut && !r.counts.cancelled) continue;
    console.error(`worker ${r.i + 1} ran out of time: its shard exceeded the ${(TIMEOUT / 1000).toFixed(0)} s TEST_TIMEOUT after ${r.seconds.toFixed(0)} s and was stopped.`
  ...
  if (ran !== names.length) {
    console.error(`\n${names.length} tests were dealt out but ${ran} ran. Not calling that a pass.`
      + (total.cancelled || results.some(r => r.timedOut) ? ' A worker was cut short by the timeout above.' : ' That is a bug in the runner, or a worker that died.'));
    process.exit(3);
  }
  // A worker that died is a failure too, even if it never printed a test failure.
  const bad = total.fail > 0 || total.cancelled > 0 || results.some((r) => r.timedOut || r.code !== 0);
  console.log(`# result ${bad ? 'FAIL' : 'pass'}`);
  if (bad) process.exit(1);
  const features = await runProcess(['--test', '--test-reporter=spec', require.resolve('./features.test.js'), require.resolve('./cues.test.js'), require.resolve('./dynamics.test.js'), require.resolve('./cheese-mesh.test.js'), require.resolve('./real.test.js'), require.resolve('./render-work.test.js')]);
...
module.exports = { testNames, countTests, FILE, TIMEOUT, runProcess };
