...
module.exports={name:'real',experience:'real',description:'physical prep, controls, movement and separate saves',async run(k){
 ...
 await aim([-3.3,1.1,3.07],[-3.3,2.12]);await click();k.ok(await page.evaluate(()=>realMode.world.doors.fridge),'fridge opens physically');
 await aim([-3.52,.485,3.27],[-3.3,2.12]);await click('right');k.ok(await page.evaluate(()=>realMode.heldEntity()?.kind==='meat'),'one mince pack picked up');
 ...
 await aim([-3.04,.62,-1.3],[-2.18,-1.3]);await click();k.ok(await page.evaluate(()=>realMode.world.doors.oven),'shared oven door opens');
 await aim([-3.46,.55,-1.3],[-2.18,-1.3]);await click('right');await page.waitForFunction(()=>realMode.world.entities.some(e=>e.kind==='patty'&&e.station==='oven'&&Math.abs(e.pos[1]-.544)<.003));k.near(await page.evaluate(()=>realMode.world.entities.find(e=>e.kind==='patty').pos[1]),.544,.003,'food rests on the oven rack');await k.shot('oven-transfer');
