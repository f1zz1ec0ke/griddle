...
module.exports={name:'real-refinement',experience:'real',description:'thin-food pickup, bun-first builds, parked probes, water and settled layers',async run(k){
 ...
 const probe=await page.evaluate(()=>{
  ...
  spatula.held=false;r.held=null;r.renderEntities(0);r.hover={data:{type:'entity',entity:tool.id},point:new T.Vector3(...tool.pos)};r.toggleGrab();check('right click retrieves the probe',r.held===tool.id&&!tool.probeAttachment);r.action=null;r.placeHeld({data:{type:'rest',entity:tool.id},point:new T.Vector3(...tool.home)});r.renderEntities(0);r.hint();return out;
