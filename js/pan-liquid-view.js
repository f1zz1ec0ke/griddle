...
(function(root){
  ...
  class WaterView{
    ...
    surfaceHeight(x,z,f){const r=Math.hypot(x,z),wave=(Math.sin(r*330-this.time*12)*this.pour*.00035+Math.sin(x*510+this.time*9)*Math.cos(z*460-this.time*7)*this.heat*.00013)*Math.sin(f*Math.PI);return this.depth+wave+.0003*f**14;}
    heightAt(x,z){if(!this.group.visible)return 0;const f=Math.hypot(x,z)/this.boundary(Math.atan2(z,x));return f>=1?0:(this.surfaceHeight(x,z,f)+.00007)*clamp((1-f)*20,0,1);}
    update(pan,time,floorY,enabled=true){
      ...
      for(let i=0;i<a.count;i++){
        const ring=i?1+Math.floor((i-1)/segments):0,angle=i?((i-1)%segments)/segments*Math.PI*2:0,f=ring/rings,r=f*this.boundary(angle),x=Math.cos(angle)*r,z=Math.sin(angle)*r;
      ...
      if(this.bubbles.visible){const d=this.dummy;for(let i=0;i<32;i++){const angle=i*2.399,r=Math.sqrt((i+.5)/32)*this.radius*.88,u=(time*(1.1+heat)+i*.618)%1,scale=(.001+u*.003)*heat;d.position.set(Math.cos(angle)*r,this.depth+.00035+Math.sin(u*Math.PI)*.0008,Math.sin(angle)*r);d.rotation.set(-Math.PI/2,0,angle);d.scale.setScalar(scale*Math.sin(u*Math.PI));d.updateMatrix();this.bubbles.setMatrixAt(i,d.matrix);}this.bubbles.instanceMatrix.needsUpdate=true;}
    }
  }
